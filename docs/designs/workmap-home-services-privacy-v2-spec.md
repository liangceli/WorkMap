# WorkMap Homepage - Services and Privacy V2
...
- Real Virtual Office reference: `docs/designs/screen.png`
...
- Pixel avatar: `workmap/apps/web/public/assets/avatars/presets/Farmer_2_No_Hat_32x32.png`
...
Desktop copy occupies columns 1-5 and real product media columns 6-12. The map crop must keep People, room context, map, and running status readable. Do not add metrics, charts, fake people, fake rooms, or substitute controls.
...
Mobile places copy before a deliberate `16 / 10` map crop. Never shrink the entire desktop screenshot until controls are unreadable.
...
- Product frame reveals left-to-right and scales `1.025 -> 1` over `900ms`.
...
Use modern line icons for browser, desktop, queue, key, and report concepts. Existing pixel avatar, handbook, and sign may punctuate the route at small integer scale. Do not create pixel hardware or furniture. Desktop uses an orthogonal route; mobile uses a top-to-bottom route.
...
The Amber band mask-reveals over `650ms`. One real avatar sprite can play one short existing sequence once; do not generate frames or loop continuously.

## FAQ

Desktop uses a `36% / 64%` grid. The left panel uses `position: sticky; top: 112px; align-self: start`. Keep native page scrolling: do not intercept the mouse wheel. The left panel remains fixed while the right list passes it; after item ten, the page continues naturally. Mobile places the intro normally above full-width accordions.
