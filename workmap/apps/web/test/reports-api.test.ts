...
import { getAgentLiveStatus, getTrackingAudit, getUsageSummary } from "../lib/api/reportsApi.js";
...
test("live Agent polling can skip the periodic summary revision query", async () => {
  const originalFetch = globalThis.fetch;
  let requestedUrl = "";
  globalThis.fetch = async (input) => {
    requestedUrl = String(input);
    return new Response(JSON.stringify({ scope: "user", agentStatus: null, browserExtensionCoverage: [], activityRevision: null }), {
      status: 200,
      headers: { "Content-Type": "application/json" },
    });
  };
  try {
    ...
    assert.equal(result.ok, true);
    assert.match(requestedUrl, /includeRevision=false/);
  } finally {
    globalThis.fetch = originalFetch;
  }
});

test("reports API can request a fast summary without inline live or audit history", async () => {
  const originalFetch = globalThis.fetch;
  let requestedUrl = "";
  globalThis.fetch = async (input) => {
    requestedUrl = String(input);
    return new Response(JSON.stringify({ scope: "user", userId: "11111111-1111-4111-8111-111111111111", departmentId: null, range: { from: "2026-06-01", to: "2026-06-21", timeZone: "UTC" }, apps: [], websites: [], daily: [] }), {
      status: 200,
      headers: { "Content-Type": "application/json" },
    });
  };
  try {
    ...
    assert.equal(result.ok, true);
    assert.match(requestedUrl, /includeAudit=false/);
    assert.match(requestedUrl, /includeLive=false/);
  } finally {
    globalThis.fetch = originalFetch;
  }
});

test("tracking audit is requested separately from the report summary", async () => {
  const originalFetch = globalThis.fetch;
  let requestedUrl = "";
  globalThis.fetch = async (input) => {
    requestedUrl = String(input);
    return new Response(JSON.stringify({ scope: "user", userId: "11111111-1111-4111-8111-111111111111", agentSessions: [], deviceStatusHistory: [], appTimeline: [] }), {
      status: 200,
      headers: { "Content-Type": "application/json" },
    });
  };
