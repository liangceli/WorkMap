...
test("MV3 runtime listens to tab, window, idle and alarm lifecycle", async () => {
  ...
  for (const marker of ["tabs.onActivated", "tabs.onUpdated", "windows.onFocusChanged", "idle.onStateChanged", "alarms.onAlarm", "runtime.onStartup"]) assert(source.includes(marker));
